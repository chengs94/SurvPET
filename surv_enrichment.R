  ...
  if (!acc.fu){
    ...
    if (length(end.of.trial) > 1){
      for (j in 1:length(end.of.trial)){
        ...
        if (comp.cost & !is.null(cost.keeping)) colnames(tab[[j]]) <- cnames[1:10]
        if (comp.cost & is.null(cost.keeping)) colnames(tab[[j]]) <- cnames[c(1:8,10)]
        if (!comp.cost) colnames(tab[[j]]) <- cnames[1:7]
      }
    ...
    if (length(end.of.trial) == 1){
      tab <- cbind(selected.biomarker.quantiles, eprob, esd, npat, sd.npat, nscr.orig, sd.nscr.orig,
                   cost, sd.cost, reduc)
      if (comp.cost) colnames(tab) <- cnames[1:10]
      if (!comp.cost) colnames(tab) <- cnames[1:7]
    }
  }
